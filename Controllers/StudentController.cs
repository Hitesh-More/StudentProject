...
namespace StudentProject.Controllers
{
    public class StudentController : Controller
    {
        ...
        /// <summary>
        /// here student can see his/her teacher 
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public ActionResult MyTeacher()
        {
            var temp = Convert.ToString(Session["EmailInfo"]);
            //for that list to show
            List<UserRegistrationViewModel> objUserRegistrationViewModelList = new List<UserRegistrationViewModel>();
            try
            {
                var testCourse= db.UserRegistrations.Where(w => w.Email == temp)
                            .Select(q => q.CourseId).FirstOrDefault();

                var subjects = (from item in db.SubjectInCourses
                                join c in db.Subjects on item.SubjectId equals c.SubjectId
                                join d in db.Courses on item.CourseId equals d.CourseId
                                where item.CourseId == testCourse
                ...
                var result = (from item in db.UserRegistrations
                              ...
                              join e in db.SubjectInCourses on cx.CourseId equals e.CourseId
                              join d in db.Subjects on e.SubjectId equals d.SubjectId
                              where cy.RoleName == "Teacher"
                              where cx.CourseId== testCourse
                              select new UserRegistrationViewModel
                              {
                                  UserId = item.UserId,
                                  Fname = item.Fname,
        ...
        {
            var temp = Convert.ToString(Session["EmailInfo"]);
            var result = db.UserRegistrations.Where(w=>w.Email==temp)
                            .Select(q=>q.CourseId).FirstOrDefault();
            var subjects = (from item in db.SubjectInCourses
                            join c in db.Subjects on item.SubjectId equals c.SubjectId
                            join d in db.Courses on item.CourseId equals d.CourseId
                            where item.CourseId == result
                            select new UserRegistrationViewModel
                            {
